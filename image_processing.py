...
def threshimg(imagepath):
    im = cv2.imread(imagepath, 0)
    cv2.imwrite(imagepath, im)

if __name__ == "__main__":
    img = str(sys.argv[1])
    out_img = img[:img.index('.')]+".crop"+img[img.index('.'):]
    process_image(img, out_img)
    threshimg(out_img)
    print(get_text(out_img))
